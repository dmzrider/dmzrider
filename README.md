<!-- DYNAMIC BANNER -->
<div align="center">
  <img src="banner.svg" width="100%" alt="Manu's Dynamic Cyber Banner" />
</div>

<!-- ⚡ TYPEWRITER WIDGET -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Outfit&size=24&duration=3000&pause=1000&color=00F0FF&center=true&vCenter=true&width=800&lines=Discord+Bot+Developer;SA-MP+%2F+Open.MP+Pawn+Scripter;Systems+Security+Engineer;DDoS+Alert+%26+Packet+Shield+Dev;Freelance+Startup+Developer" alt="Typing SVG" />
</div>

<br/>

<!-- 🟢 HUD STATUS BAR -->
<div align="center">
  <img src="hud-status.svg" width="100%" alt="Availability HUD Status" />
</div>

<br/>

<!-- 📊 LIVE COUNTERS & BADGES -->
<div align="center">
  <img src="https://komarev.com/ghpvc/?username=dmzrider&style=flat-square&color=00F0FF&label=PROFILE+VIEWS" alt="Profile Views" />
  &nbsp;&nbsp;
  <img src="https://img.shields.io/github/followers/dmzrider?style=flat-square&color=a855f7&label=FOLLOWERS" alt="Followers" />
  &nbsp;&nbsp;
  <img src="https://img.shields.io/github/stars/dmzrider?style=flat-square&color=ff007f&label=TOTAL+STARS" alt="Total Stars" />
  &nbsp;&nbsp;
  <a href="https://discord.com">
    <img src="https://img.shields.io/badge/Discord-echo__dev-5865F2?style=flat-square&logo=discord&logoColor=white" alt="Discord" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/dmzrider">
    <img src="https://img.shields.io/badge/GitHub-dmzrider-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/dmzrider/dmzrider/actions">
    <img src="https://img.shields.io/github/actions/workflow/status/dmzrider/dmzrider/update-profile-stats.yml?branch=main&style=flat-square&color=22c55e&label=PROFILE+ENGINE" alt="Profile Engine" />
  </a>
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-about.svg" width="100%" alt="About Me Section" /></div>

<br/>

<div align="center">
  <img src="about-terminal.svg" width="100%" alt="About Me Terminal" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-building.svg" width="100%" alt="Currently Building Section" /></div>

<br/>

<div align="center">
  <img src="build-log.svg" width="100%" alt="Currently Building Log" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-achievements.svg" width="100%" alt="Achievements Section" /></div>

<br/>

<div align="center">
  <img src="kpi-achievements.svg" width="100%" alt="KPI Achievements Dashboard" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-tech.svg" width="100%" alt="Tech Stack Section" /></div>

<br/>

<div align="center">
  <table border="0" cellspacing="0" cellpadding="10">
    <tr>
      <td align="center" width="165"><b>🤖 Automation &amp; Bots</b></td>
      <td align="center" width="285">
        <img src="https://skillicons.dev/icons?i=js,ts,nodejs,express" alt="Bot Stack" />
      </td>
      <td width="370">Custom Discord bots, permission workflows, webhook piping, interactive buttons &amp; modals.</td>
    </tr>
    <tr>
      <td align="center"><b>🎮 Game Development</b></td>
      <td align="center">
        <img src="https://img.shields.io/badge/Pawn-Open.MP%20%2F%20SA--MP-3b82f6?style=for-the-badge&logo=cplusplus&logoColor=white" alt="Pawn" />
      </td>
      <td>Modular gamemode scripting with y_less libs, MySQL threading, tick optimization, 3.6 MB codebase (AKRP-V5).</td>
    </tr>
    <tr>
      <td align="center"><b>🗄️ Database &amp; Infra</b></td>
      <td align="center">
        <img src="https://skillicons.dev/icons?i=sqlite,mysql,linux,docker" alt="Database" />
      </td>
      <td>Database normalization, secure pipeline configs, localized hosting setups, and containerization.</td>
    </tr>
    <tr>
      <td align="center"><b>🛡️ Cloud &amp; Security</b></td>
      <td align="center">
        <img src="https://skillicons.dev/icons?i=cloudflare,bash,git,vscode" alt="Security Tools" />
      </td>
      <td>Network traffic shielding, DDoS alert automation via bash triggers, Cloudflare proxying.</td>
    </tr>
  </table>
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-projects.svg" width="100%" alt="Projects Section" /></div>

<br/>

<div align="center">
  <img src="arcade-projects-v5.svg" width="100%" alt="Arcade Project Showcase" />
</div>

> 💡 *AKRP-V5-MAIN and MazhaV1 are in private repos — public releases coming soon!*

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-analytics-v2.svg" width="100%" alt="Analytics Section" /></div>

<br/>

<div align="center">
  <table border="0">
    <tr>
      <td align="center" valign="top">
        <img src="https://github-stats-extended.vercel.app/api?username=dmzrider&show_icons=true&title_color=00f0ff&icon_color=ff007f&text_color=e2e8f0&bg_color=070a13&hide_border=true&count_private=true" height="200px" alt="GitHub Stats" />
      </td>
      <td align="center" valign="top">
        <img src="https://github-stats-extended.vercel.app/api/top-langs/?username=dmzrider&layout=compact&title_color=ff007f&icon_color=00f0ff&text_color=e2e8f0&bg_color=070a13&hide_border=true" height="200px" alt="Top Languages" />
      </td>
    </tr>
  </table>
</div>

<br/>

<!-- ⚡ REPOSITORY LANGUAGE TELEMETRY (auto-updated by workflow) -->
<div align="center">

<!-- START_SECTION:languages -->
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 495 220" width="495">
  <defs>
    <linearGradient id="wkbg" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#070a13"/>
      <stop offset="100%" stop-color="#0d1117"/>
    </linearGradient>
    <linearGradient id="barPawnSAMP" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#3b82f6"/><stop offset="100%" stop-color="#60a5fa"/>
    </linearGradient>
    <linearGradient id="barJavaScript" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#f7df1e"/><stop offset="100%" stop-color="#f1e05a"/>
    </linearGradient>
    <linearGradient id="barTypeScript" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#3178c6"/><stop offset="100%" stop-color="#60a5fa"/>
    </linearGradient>
    <linearGradient id="barSQLMySQL" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#00a3cc"/><stop offset="100%" stop-color="#00e5ff"/>
    </linearGradient>
    <linearGradient id="barPython" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#3572A5"/><stop offset="100%" stop-color="#4584b6"/>
    </linearGradient>
    <filter id="wkGlow"><feGaussianBlur stdDeviation="2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
  </defs>
  <rect width="495" height="220" rx="10" fill="url(#wkbg)" stroke="#1a233a" stroke-width="1.2"/>
  <rect width="495" height="220" rx="10" fill="none" stroke="#a855f7" stroke-width="1.2" opacity="0.3">
    <animate attributeName="opacity" values="0.2;0.6;0.2" dur="3s" repeatCount="indefinite"/>
  </rect>

  <!-- Title -->
  <text x="30" y="32" font-family="'Courier New',Courier,monospace" font-size="12" font-weight="bold" fill="#a855f7" letter-spacing="1">⚡ REPOSITORY LANGUAGE METRICS</text>
  <text x="465" y="31" font-family="'Courier New',Courier,monospace" font-size="9" fill="#64748b" text-anchor="end">Metrics: Dynamic Source</text>

  <!-- Pawn (SA-MP) -->
  <g transform="translate(30, 48)">
    <text x="0" y="12" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#e2e8f0">Pawn (SA-MP)</text>
    <text x="100" y="12" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3b82f6">3.6 MB</text>
    <rect x="160" y="3" width="220" height="9" rx="4.5" fill="#161b22"/>
    <rect x="160" y="3" width="0" height="9" rx="4.5" fill="url(#barPawnSAMP)">
      <animate attributeName="width" values="0;193;193" dur="1.5s" fill="freeze" begin="0.2s"/>
    </rect>
    <text x="390" y="11" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3b82f6">88%</text>
  </g>
  <!-- JavaScript -->
  <g transform="translate(30, 76)">
    <text x="0" y="12" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#e2e8f0">JavaScript</text>
    <text x="100" y="12" font-family="'Courier New',Courier,monospace" font-size="10" fill="#f7df1e">235 KB</text>
    <rect x="160" y="3" width="220" height="9" rx="4.5" fill="#161b22"/>
    <rect x="160" y="3" width="0" height="9" rx="4.5" fill="url(#barJavaScript)">
      <animate attributeName="width" values="0;13;13" dur="1.5s" fill="freeze" begin="0.4s"/>
    </rect>
    <text x="390" y="11" font-family="'Courier New',Courier,monospace" font-size="10" fill="#f7df1e">6%</text>
  </g>
  <!-- TypeScript -->
  <g transform="translate(30, 104)">
    <text x="0" y="12" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#e2e8f0">TypeScript</text>
    <text x="100" y="12" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3178c6">150 KB</text>
    <rect x="160" y="3" width="220" height="9" rx="4.5" fill="#161b22"/>
    <rect x="160" y="3" width="0" height="9" rx="4.5" fill="url(#barTypeScript)">
      <animate attributeName="width" values="0;8;8" dur="1.5s" fill="freeze" begin="0.6s"/>
    </rect>
    <text x="390" y="11" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3178c6">4%</text>
  </g>
  <!-- SQL/MySQL -->
  <g transform="translate(30, 132)">
    <text x="0" y="12" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#e2e8f0">SQL/MySQL</text>
    <text x="100" y="12" font-family="'Courier New',Courier,monospace" font-size="10" fill="#00a3cc">80 KB</text>
    <rect x="160" y="3" width="220" height="9" rx="4.5" fill="#161b22"/>
    <rect x="160" y="3" width="0" height="9" rx="4.5" fill="url(#barSQLMySQL)">
      <animate attributeName="width" values="0;4;4" dur="1.5s" fill="freeze" begin="0.8s"/>
    </rect>
    <text x="390" y="11" font-family="'Courier New',Courier,monospace" font-size="10" fill="#00a3cc">2%</text>
  </g>
  <!-- Python -->
  <g transform="translate(30, 160)">
    <text x="0" y="12" font-family="Arial,sans-serif" font-size="11" font-weight="bold" fill="#e2e8f0">Python</text>
    <text x="100" y="12" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3572A5">26 KB</text>
    <rect x="160" y="3" width="220" height="9" rx="4.5" fill="#161b22"/>
    <rect x="160" y="3" width="0" height="9" rx="4.5" fill="url(#barPython)">
      <animate attributeName="width" values="0;2;2" dur="1.5s" fill="freeze" begin="1.0s"/>
    </rect>
    <text x="390" y="11" font-family="'Courier New',Courier,monospace" font-size="10" fill="#3572A5">1%</text>
  </g>
</svg>
<!-- END_SECTION:languages -->

</div>

<br/>

<div align="center">
  <img src="profile-streak-stats-v2.svg" alt="GitHub Streak Stats" />
</div>

<br/>

<div align="center">
  <img src="activity-graph-v2.svg" width="100%" alt="Activity Graph" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-activity.svg" width="100%" alt="Activity Section" /></div>

<br/>

<div align="center">
  <img src="activity-radar-v2.svg" width="100%" alt="Activity Radar" />
</div>

<br/>

<div align="center">
  <img src="diagnostics-v2.svg" width="100%" alt="Diagnostics" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-snake.svg" width="100%" alt="Snake Game Section" /></div>

<br/>

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="github-contribution-grid-snake-dark-v2.svg">
    <source media="(prefers-color-scheme: light)" srcset="github-contribution-grid-snake-v2.svg">
    <img alt="GitHub Contribution Snake Grid" src="github-contribution-grid-snake-v2.svg" />
  </picture>
</div>

<br/>

---

<!-- 💡 QUOTE & JOKE -->
<div align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Developer Quote" />
</div>

<br/>

<div align="center">
  <img src="https://readme-jokes.vercel.app/api?hideBorder&theme=tokyonight&bgColor=%23070a13&textColor=%23e2e8f0&aColor=%2300f0ff&qColor=%23a855f7" alt="Dev Joke of the Day" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-global.svg" width="100%" alt="Global Reach Section" /></div>

<br/>

<div align="center">
  <img src="global-reach.svg" width="100%" alt="Global Reach Map" />
</div>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-specialties.svg" width="100%" alt="Specialties Section" /></div>

<br/>

<div align="center">
  <img src="specialties-grid.svg" width="100%" alt="Specialties Grid" />
</div>

<br/>

---

<!-- ============================================================ -->
<div align="center"><img src="sec-connect.svg" width="100%" alt="Connect Section" /></div>

<br/>

<div align="center">
  <img src="contact-cards.svg" width="100%" alt="Contact Cards" />
</div>

<br/>

---

<!-- FOOTER -->
<div align="center">
  <img src="footer-bar.svg" width="100%" alt="Footer" />
</div>
